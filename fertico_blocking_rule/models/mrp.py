...
class MrpProduction(models.Model):
    ...
    def open_produce_product(self):
        self.ensure_one()
        if self.move_raw_ids.filtered(lambda ml: ml.product_uom_qty != ml.reserved_availability or ml.quantity_done > ml.reserved_availability):
    ...
    def button_plan(self):
        for production in self:
            if production.move_raw_ids.filtered(lambda ml: ml.product_uom_qty != ml.reserved_availability or ml.quantity_done > ml.reserved_availability):
